...
public class JavaPracticeEmployees {
    public static void main(String[] args) {
        List<Employee> employees = DataBase.getEmployees();
        // 1. Retrieve Employee Details
        Map<String, LocalDate> nameToBirthdayMap = employees.stream().collect(Collectors.toMap(Employee::getName, Employee::getBornDate, (existing, replacement) -> existing));
        ...
        Map<String, LocalDate> itEmployees = employees.stream().filter(emp -> "IT".equals(emp.getDepartment())).collect(Collectors.toMap(Employee::getName, Employee::getBornDate, (existing, replacement) -> existing));
        System.out.println("\nIT Department Employees - Name and Birthday:");
        itEmployees.forEach((name, dob) -> System.out.println(name = " - " + dob));

        // 3. Check for Duplicate Names
        Set<String> allNames = new HashSet<>();
        Set<String> duplicateNames = employees.stream().map(Employee::getName).filter(name -> !allNames.add(name)).collect(Collectors.toSet());
        ...
        Map<String, Long> maritalStatusCount = employees.stream().collect(Collectors.groupingBy(Employee::getMaritalStatus, Collectors.counting()));
        ...
        employees.stream().filter(emp -> emp.getHireDate() != null && emp.getHireDate().getYear() < 2010).map(Employee::getName).forEach(System.out::println);

        // 6. list out name out of female employee born before 2000
        System.out.println(" name of femele Employee who joined 2000");
        employees.stream().filter(emp -> emp.getHireDate() != null && emp.getHireDate().getYear() == 2000).filter(emp -> "female".equalsIgnoreCase(emp.getSex()))
                .map(Employee::getName).forEach(System.out::println);

        //7.give the name and id of a martial employee
        ...
        // 8.give account of total no of employee
        System.out.println("\ngive account of total no of employee:");
        long totelEmployee = employees.stream().count();
        System.out.println(totelEmployee);

        // 9.accociate name or male employee with their job title
        ...
        employees.stream().sorted(Comparator.comparing(Employee::getCtc).reversed()).forEach(emp -> System.out.println(emp.getName() + "-" + emp.getCtc()));
