...
public class ParkingManagement1 {
	...
	Map<String, LocalTime> parking = new LinkedHashMap<>();
	...
	Runnable entryofvehicle = () -> {
		...
		while (true) {
			...
			try {
				...
				LocalTime entrytime = LocalTime.now().minusMinutes(randomMinuteAgo);
	...
	Runnable exitOfvehicle = () -> {
		...
		while (true) {
			...
			try {
				...
				LocalTime entryTime = parking.remove(vehicle);
	...
	private long calculatewaitcarParkingCost(LocalTime entryTime, LocalTime exitTime) {
